...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		engranajeAngle += 0.8f * (deltaTime / 61.0f);
		...
		*/
		...
		// Engranajes - mismo origen que el proyector, cafe clarito, rotando en propio eje
		color = glm::vec3(0.8f, 0.55f, 0.3f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		// T_mundo * R * T_centro-inverso * S  =>  gira alrededor del centro del modelo
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 5.0f));   // posicion mundo
		model = glm::rotate(model, engranajeAngle, glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Engranaje1_M.RenderModel();
		Engranaje2_M.RenderModel();
		Engranaje3_M.RenderModel();
		EngranajeAdicional1_M.RenderModel();
		EngranajeAdicional2_M.RenderModel();
		EngranajeAtras1_M.RenderModel();
		EngranajeAtras2_M.RenderModel();
		EngranajeProyector1_M.RenderModel();
		EngranajeProyector2_M.RenderModel();
